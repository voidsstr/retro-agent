...
checked on the host against the monitor's EDID ranges at the HIGHEST refresh
the driver lists there (mode_sweep.py's gate: `vcrctl info` and `vcrctl
modes`; under another driver --monitor-info, used once the box's registry
says that monitor is the one on it, or with --i-have-checked-the-monitor
when it cannot say). The EXECW adds the pace gate's worst case for the two
switches (a busy switch lock, then the floor) to --timeout, and a budget past
the agent's clamp is refused.
...
cleaned up before this returns, and fails: PROCLIST, a ddlab.exe that was not
...
("focus_lost") or found the switch lock busy ('pace lock busy') fails too.
"""
import argparse
import asyncio
import json
...
async def main_async(a):
    ...
    if full and getattr(a, "test_bed", False):
        ...
        if why:
            return refused(a, why)
        print(f"plan: ddlab {a.mode} at {modes[0]} (the highest refresh the driver lists there: "
              f"refresh 0 lets it pick)  {ms.rates(calc, modes[0])}\n  monitor "
              f"{ms.describe(rng)}", flush=True)
    ...
    trouble = marker = None
    try:
        reply = await call(a, f'EXECW {budget} cmd /c start "ddlab" /wait "{DIR}\\ddlab.exe" '
                           f'{args}', timeout=budget + ms.HOST_SLACK_S)
...
def main():
    ...
    ap.add_argument("--monitor-info", help="flip/blt under another driver: a `vcrctl info` saved "
