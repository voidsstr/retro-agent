...
| G3 | **No lane of our Glide fork bounds its FIFO `makeRoom` / `grSstIdle` spins** (no `WEDGE` bound anywhere in the fork). The vintage glide2 got WEDGE-BREAK bounds in `retro-3dfx` `2b3e832` (2026-08-03). A stalled chip spins forever — a candidate contributor to the V5 6000 freeze | 2026-09-23 |
...
| # | Defect | Where | Effect |
|---|---|---|---|
| I1 | **Refresh is hard-coded to 60 Hz**, and the 0.1.34 fix (monitor-max refresh) and 0.1.35 software cursor **exist in no source** — only in the CHANGELOG and in their tests | `fxapi.c` ~342 | Every fullscreen game runs at 60 Hz unless a Glide refresh override is set (`FX_GLIDE_REFRESH` on h3/h5, `SSTV2_REFRESH_*` on cvg) |
