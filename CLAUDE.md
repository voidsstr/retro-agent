...
| **Kernel driver pair — `vcr-kmd`** (2026-09-25, supersedes `vcr-disp`) | OUR code: XDDM miniport `vcrmp.sys` + GDI display DLL `vcrdd.dll`, mingw-built, every import checked against XP SP3; flight recorder (ring found in crash dumps by `vcrdump`/`vcrctl dump`), flushed boot phases, boot-loop breaker + `Diag\Disable` → VGA fallback | open register sources (Glide GPL headers, tdfxfb, xf86-video-tdfx) + golden captures from the vendor driver; vintage H5 source read for guidance only | `voodoo-cleanroom/vcr-kmd/` ([README](voodoo-cleanroom/vcr-kmd/README.md)) | **drives the V5 6000 on `.124` (2026-09-26)**: 123/123 vendor modes verified on silicon (registers read back = AmigaMerlin's); **Quake II on our ICD + Glide + kernel driver = the whole stack ours, 146.8 fps 1 chip (147.9 over AmigaMerlin's kernel)**; safety net + crash dump proven in the QEMU test bed; **4-chip SLI works (2026-09-26)**: SLI config space identical to AmigaMerlin's (4 chips + the bridge's clock GPIO), Quake II 4 chips 201.3 fps 640x480 / 98.2 fps 1600x1200@60 Hz = parity with AmigaMerlin's kernel; **DDC/EDID** reads the monitor and filters the mode list by its ranges (GAMERES sees it again); the AA configs next |
...
- **Where it runs (2026-09-24): the Voodoo 5 6000 on `.124`.** Our ICD over
  ...
  Quake II**. **Since 2026-09-26 the kernel driver is ours too:** `vcr-kmd`
  (`voodoo-cleanroom/vcr-kmd/`, miniport + display DLL with 2D engine,
  DirectDraw and a DX7 D3D HAL) runs `.124`, and Quake II on our ICD + our h5
  Glide + our kernel driver ran 173.4 fps at 1024x768x16 in 4-chip SLI
  (AmigaMerlin's best: 147-173). Our Glide on our kernel matches AmigaMerlin's
  4-chip fill rate (1124.6 vs 1124.5 Mpix/s). **Every live mode switch goes
  through `vcr-kmd/tools/vcr_pace.h`** (the monitor is a 1998 CRT - see the
  v56k plan). `.171`'s Voodoo 2 lane (`deploy/deploy171.py`) is unchanged.
  Rows: README §13.3 and `docs/v56k-benchmark-plan.md`.
