#!/usr/bin/env bash
# Install all game servers (UT2004 + UT99 + Quake 2 + OpenArena/Q3 + CS 1.6).
...
bash "$HERE/install-cs16-server.sh"

echo
echo "All servers installed. Status:"
...
Router port-forwards needed (UDP, all → this host's wired LAN IP):
  ...
  27910                Quake 2
  27960                OpenArena / Q3
  27015                Counter-Strike 1.6 (LAN-only by default — sv_lan 1)
...
External reachability test (run from a different network / mobile hotspot):
  ...
  printf '\xff\xff\xff\xffTSource Engine Query\x00' | nc -u -w3 <public.ip> 27015  # CS 1.6

Masters will re-probe and list the servers within 15–30 minutes.
(CS 1.6 ships LAN-only — needs a Steam GSLT + sv_lan 0 for public listing.)
TAIL
