...
| UT99 | 1 | UnrealScript mutator (`adapters/ut99/`) — no native plugin ABI, but the bot AI is itself UnrealScript. `TcpLink` never connects on this OldUnreal 469e Linux build, and raw binary over `UdpLink` never delivers real content back either (`ReceivedBinary`'s buffer is always garbage) — but hex-text over `UdpLink` works, verified live end to end: real bidirectional traffic, a climbing decision count in policyd, and a control experiment showing bots freeze under a no-op policy and resume under their own AI when disabled. Full account in `adapters/ut99/README.md` | **done, verified live** |
