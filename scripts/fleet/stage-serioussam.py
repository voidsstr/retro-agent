...
#: The three launchers are NO LONGER GENERATED HERE. They come from the fleet's
#: one canonical mount launcher - provisioning/discmount/mount-launcher-template.bat
#: plus provisioning/discmount/specs/SeriousSam*-{Play,Host,Join}.json - the same
#: generator ten other disc titles use:
#:
#:     python3 scripts/fleet/make-mount-launcher.py \
#:         --spec provisioning/discmount/specs/SeriousSamFirstEncounter-Play.json \
#:         --out  "<library>/SeriousSamFirstEncounter/Play Serious Sam - The First Encounter.bat"
#:
#: This script owned a MOUNTDISC.BAT that was the same logic factored into one
#: CALLable file per title, and a locked-DAEMON-Tools-unit fix had to be applied
#: to BOTH - which is exactly the drift a second copy guarantees. It is gone.
#:
#: What stays here is what the generator has no opinion about: launch.txt, the
#: icon, requires.json, NOTES.txt, and removing per-box state from the tree.
_REMOVED_PLAY_TEMPLATE = r'''@echo off
...
WHAT THIS TITLE NEEDS THAT MOST DO NOT
    ...
    every one of this title's launchers mounts it before starting anything.
    Those launchers are GENERATED - they are the fleet's one canonical mount
    launcher (provisioning/discmount/mount-launcher-template.bat) plus this
    title's spec (provisioning/discmount/specs/%(dir)s-{Play,Host,Join}.json)
    in the retro-agent repo. Do not hand-edit one; fix the spec and regenerate,
    or the next disc title you touch will disagree with this one. The image is the retail disc,
...
#: script used to ship - is gone rather than lingering beside them.
LAUNCHER_MARKS = (
    ':md_finddisc' , # NOT this: the old MOUNTDISC label. Asserted ABSENT below.
)
#: Present in every launcher the canonical template generates.
TEMPLATE_MARKS = (
    'set "REQUIREDISC=',
    'start "" /b "%DT%" -mount 0,',
    'if not defined DISCDRV taskkill /f /im daemon.exe',
)
#: The file this script used to ship. Its presence means a stale second copy of
#: the mount logic is still on the share or the box.
SUPERSEDED = ('MOUNTDISC.BAT',)
...
def main():
    ...
    for name, t in sorted(TITLES.items()):
        ...
        for fn in (t['play'], t['host'], t['join']):
            lp = os.path.join(tree, fn)
            if not os.path.isfile(lp):
                print('%-28s %-46s ABSENT - generate it with '
                      'make-mount-launcher.py' % (name, fn))
                rc = 1
                continue
            lb = open(lp, 'rb').read().decode('latin1')
            missing = [m for m in TEMPLATE_MARKS if m not in lb]
            if missing:
                print('%-28s %-46s NOT FROM THE TEMPLATE (missing %s)'
                      % (name, fn, ', '.join(repr(m) for m in missing)))
                rc = 1
            elif any(m in lb for m in LAUNCHER_MARKS):
                print('%-28s %-46s still calls the old MOUNTDISC logic'
                      % (name, fn))
                rc = 1
            else:
                print('%-28s %-46s ok (generated from the template)' % (name, fn))
