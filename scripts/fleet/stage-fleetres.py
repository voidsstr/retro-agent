...
#: Markers that identify a launcher generated by the DISC-MOUNT generator
#: (provisioning/discmount/mount-launcher-template.bat + a spec). Detected by
#: CONTENT rather than by a filename list, so it also protects titles added
#: after this was written - a list here would go stale the first time a spec is
#: added, which is the same class of failure one level up.
#:
#: THIS COLLISION HAS ALREADY HAPPENED. new_launcher() writes a launcher from
#: scratch and put() overwrites whenever the content differs, so a run of this
#: tool replaced Jedi Academy's 22 KB generated mount launcher with a 2 KB one
#: carrying NO DISC MOUNT AT ALL - silently, on a title whose entire fix was the
#: mount, and with both tools reporting success. Two mechanisms claimed
#: `<title>/Play <name>.bat` and nothing said so.
DISCMOUNT_MARKERS = ('set "REQUIREDISC=', 'call :finddisc', ':waitdisc')


def is_discmount_launcher(path):
    """True if this .bat is one the disc-mount generator owns."""
    ...
    try:
        ...
        return False
    return all(m in body for m in DISCMOUNT_MARKERS)
...
TITLES = {
    ...
    #
    # THESE ENTRIES NOW EXIST TO STAGE THE PAYLOAD, NOT TO PATCH LAUNCHERS.
    # Serious Sam's three launchers per title are generated by
    # make-mount-launcher.py, and the block below is embedded in each spec's
    # `fleetres_block` - so patch_launcher() sees the FLEETRES call already
    # present and correctly skips. What the entry still does, and what the
    ...
    # first dies on a black screen no matter what resolution it was handed.
    ...
    "JediAcademy": {
        ...
        # DISC-MOUNT launchers generated from provisioning/discmount/specs/,
        # and their FLEETRES block lives in those specs' `fleetres_block` -
        # including the r_mode -1 / r_customwidth cfg writes and the matching
        # +set command line. A "new" recipe here would write a launcher from
        # scratch OVER the generated one, replacing 22 KB that mounts the disc
        # with 2 KB that does not, on a title whose entire fix was the mount.
        # That is not hypothetical - it happened on 2026-08-31 and is what
        # is_discmount_launcher() now refuses.
        "launch_txt": [
...
class Runner:
    ...
    def new_launcher(self, tdir, title, name, spec):
        exe, mod = spec
        path = os.path.join(tdir, name)
        # NEVER clobber a disc-mount launcher. This tool would replace it with
        # a minimal one that does not mount anything, and the title would then
        # ask for its CD on every box - see DISCMOUNT_MARKERS.
        if is_discmount_launcher(path):
            self.fail(
                "%s/%s is a DISC-MOUNT launcher and this tool would overwrite "
                "it with one that does not mount anything. Its FLEETRES block "
                "belongs in provisioning/discmount/specs/, not in a 'new' "
                "recipe here - drop this title's 'new' entry and regenerate "
                "with make-mount-launcher.py." % (title, name))
            return
