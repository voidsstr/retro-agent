...
WATCHDOG = "\r\n".join([
    "@echo off",
    "rem retro agent watchdog - restarts the agent when it stops SERVING, not",
    "rem merely when its process disappears.",
    "rem Installed by scripts/fleet/install-agent-watchdog.py. See that file",
    "rem for why this is a Run-key loop rather than a scheduled task, why the",
    "rem check is 90 s apart (it perturbs CPU-bound benchmarks), and why a",
    "rem process check ALONE is not enough.",
    ":loop",
    "set WD_BAD=",
    'tasklist /fi "imagename eq retro_agent.exe" /nh | find /i "retro_agent.exe" >nul',
    'if errorlevel 1 set WD_BAD=gone',
    "rem The agent can survive with its MAIN ACCEPT LOOP dead: measured three",
    "rem times on .124 - 9898 refused while 9897 was still bound and the",
    "rem process was still in tasklist, so a process-only check saw nothing",
    "rem wrong and the box needed a human. If the listener is not there, the",
    "rem agent is not an agent.",
    'if not defined WD_BAD netstat -an | find ":9898" | find /i "LISTENING" >nul',
    'if not defined WD_BAD if errorlevel 1 set WD_BAD=notlistening',
    'if defined WD_BAD (',
    '  echo %DATE% %TIME% agent %WD_BAD% - restarting >> C:\\RETRO_AGENT\\agentwd.log',
