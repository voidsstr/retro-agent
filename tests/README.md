...
| vcr-kmd (our XP kernel driver pair) flight recorder: fixed 128-byte entries behind a 16-byte magic, oldest-first across a wrap, a record torn by a crash (seq unpublished) never reported - what `vcrdump.py` relies on to read it out of a kernel dump (2026-09-25) | vcr-kmd | `native/test_vcr_kmd_log.c` + `python/test_vcr_kmd_tools.py` (C-written ring found in a fake PAGEDUMP) |
| vcr-kmd integer formatter (the display DLL may import only win32k.sys, which has no vsnprintf) (2026-09-25) | vcr-kmd | `native/test_vcr_kmd_fmt.c` |
| vcr-kmd mode math: VGA/X.org CRTC semantics, PLL within 0.5 %, 2X mode, doublescan, limits - and AGREEMENT with the vendor driver's registers captured on the V5 6000 (PLL frequency per DMT mode, no 2X below 262 MHz, vgaInit0 0x1140, CLUT bank 0) (2026-09-25) | vcr-kmd | `native/test_vcr_kmd_modes.c` |
| vcr-kmd HWCEXT ABI: every request/result offset our Glide uses (GETDEVICECONFIG numChips @40, SLI_AA dwBpp @60, PCI_OP Value @20 ...) (2026-09-25) | vcr-kmd | `native/test_vcr_kmd_abi.c` |
| vcr-kmd XP loadability: every import of vcrmp.sys / vcrdd.dll exists in XP SP3 (mingw's libntoskrnl.a offers memcmp, XP does not export it - link OK, boot fails); display DLL imports win32k.sys only; event codes never renumbered (2026-09-25) | vcr-kmd | `python/test_vcr_kmd_tools.py` |
| vcr-kmd multi-chip SLI/AA bring-up: port of Glide GPL dos_mode.c (slave BARs, shared-I/O-BAR decode toggling, bounded waits, disable); 4-chip analog SLI config space equal to AmigaMerlin 3.1-R11's on .124, slaves keep their own miscInit1 straps and carry vidPixelBufThold/vidDesktopStartAddr like the vendor's (2026-09-26) | vcr-kmd | `native/test_vcr_kmd_sli.c` |
| vcr-kmd V5 6000 external clock (ICS307 on the HiNT bridge GPIO 0xC4): our integer divider search picks exactly what Glide's floating-point search picks for every driver clock, within the part's VCO/reference limits; the 24-bit word reaches a modelled part MSB first and is latched once; the GPIO ends at 0x00222201 as under the vendor driver; a stuck clock cannot hang the sequence (2026-09-26) | vcr-kmd | `native/test_vcr_kmd_ics307.c` |
| vcr-kmd multi-chip kernel glue: IOCTL input read BEFORE the shared METHOD_BUFFERED answer is written (zeroing it first turned Glide's 4-chip enable into a disable on .124, silently); the miniport links the real 6000 clock, not the stub; every mode set turns SLI off (the teardown a dead Glide client skips); Glide is told the chips the miniport mapped (2026-09-26) | vcr-kmd | `python/test_vcr_kmd_sli_glue.py` |
| vcr-kmd monitor EDID + mode filter: E-EDID base block parsed (identity, name, 0xFD range limits incl. 1.4 offsets, preferred timing); modes outside the monitor's H/V/dot-clock ranges are not offered, with half-unit slack; a bad EDID filters nothing. Fixtures: the Dell E2414H, Gateway VX1120 and Sony CPD-G200 EDIDs from .124 (serials blanked) (2026-09-26) | vcr-kmd | `native/test_vcr_kmd_edid.c` |
| vcr-kmd monitor glue: EDID read over DDC (vidSerialParallelPort bits 18-22, VideoPortDDCMonitorHelper) BEFORE the mode list is built; the monitor child hands XP the EDID (GAMERES saw no monitor on our driver before) (2026-09-26) | vcr-kmd | `python/test_vcr_kmd_ddc_glue.py` |
| vcr-kmd DirectDraw HAL: never DDCAPS_GDI (XP probes and silently disables a HAL claiming it - DDCAPS_NOHARDWARE for every app); GCAPS_DIRECTDRAW; SRCCOPY matched by the ROP byte (the runtime passes 0x00CC0000); unsupported blits go back to the HEL; the primary is a hooked device surface with every hook in the entry table; GET_CHILD_STATE and SHARE_VIDEO_MEMORY answered, input read first (VM test bed: ddlab caps/flip/blt + GDI sweep 60/60, 2026-09-26) | vcr-kmd | `python/test_vcr_kmd_ddraw.py` |
| vcr-kmd 2D engine + DirectDraw timing (86Box Voodoo3 bed, 2026-09-26): a RECTFILL fills from the SOURCE operand, so SRCCOPY (PATCOPY filled from the pattern registers, ddlab bad_fill 4096); a source colour key needs the rop register byte 1 = D; the sync before CPU access waits for busy clear AND the PCI FIFO drained (gdilab engine/CPU interleave 170 bad -> 0); status[6] CLEAR = in vertical retrace (Glide grSstVRetraceOn); a flip is pending until latched (768 -> 62 flips/s at 60 Hz); the registers come from QUERY_PUBLIC_ACCESS_RANGES, input read first | vcr-kmd | `python/test_vcr_kmd_2d.py` |
| vcr-kmd Direct3D HAL (86Box Voodoo3 bed, 2026-09-26, d3dprobe render 26/26 windowed and fullscreen): a complex surface is named by its root in CreateSurfaceEx and the flip chain's other handles are found through the attach lists (XP re-targets with SETRENDERTARGET, it does not swap video memory - fullscreen was 18/26); a texture's pixel format is trusted without DDRAWISURF_HASPIXELFORMAT (every texture was refused); the render target is re-read per call; every DX7 DP2 opcode parsed or handed to the runtime, bounds-checked; floats only in vcrdd_3d.c inside the FPU bracket | vcr-kmd | `python/test_vcr_kmd_d3d.py` |
| vcr-kmd texture base address: the TMU addresses a texture as a mipmap whose LOD 0 is 256 wide - a 64x64 16 bpp texture's texBaseAddr is its address minus 0x28000 (86Box voodoo_recalc_tex12, Glide _grTexCalcBaseAddress); refuses non-power-of-two, >256, >8:1 and padded pitches | vcr-kmd | `native/test_vcr_kmd_texlod.c` |
| vcr-kmd fog (86Box Voodoo3, d3dprobe fogtable/fogvertex 6/6): the fog unit indexes a 64-entry table by 1/W, entry i = w 2^(3+i/4)/(8-i%4) (Glide guFogTableIndexToW); D3D linear/exp/exp2 table fog is computed on that curve with our own exp (no libm in a kernel driver), vertex fog (specular alpha) becomes a ramp table plus a synthetic 1/W per vertex; registers packed two entries + deltas each | vcr-kmd | `native/test_vcr_kmd_fog.c`, `python/test_vcr_kmd_d3d.py` |
| vcr-kmd hardware cursor pattern: a Windows mono pointer's AND/XOR pairs land in the chip's two planes, outside is transparent, hwCurLoc offset by 64 and clamped (2026-09-26; on silicon: byte-compared to the vendor's pattern, pending) | vcr-kmd | `native/test_vcr_kmd_cursor.c` |
| vcr-kmd flushed phase history survives a power cycle: DriverEntry copies the previous boot's PhaseLog/LastPhase* to Prev* before its first phase (they were overwritten, so a hung boot's history was lost); SLI/AA milestones and Glide's SLI request are phases; `vcrphases.py --prev` names the step (verified in the VM test bed; .124 deep-wedged in an AA bring-up, 2026-09-26) | vcr-kmd | `python/test_vcr_kmd_phases.py` |
...
| **ide9x / disk9x are read-only** (2026-09-25) — raw-disk readers for Win9x boxes (a second disk on .243 the BIOS could not read). ide9x talks to the secondary IDE channel's ports directly: only ECh/20h, never the primary channel, never SRST, NEW output files only, DOS device names refused. disk9x's VWIN32 INT 13h route: only reads, no AH=00h reset | `python/test_ide9x_readonly.py`, `python/test_disk9x_readonly.py` |
| **idewrite9x cannot reach another disk** (2026-09-25) — the one write-capable raw-disk tool (formatted .243's 80 GB secondary master): secondary channel only, master only, only ECh/30h/E7h, every write path IDENTIFYs first and requires an exact (byte, not locale) serial match, writes bounded by the drive's capacity; a zero run stops only via its stop file, never a kill; a named mutex keeps ide9x off the channel meanwhile; a flush's success is actually reported (`wait_done` returns the status byte) | `python/test_idewrite9x_guards.py` |
...
| **clockfix spent an unactivated XP box's whole activation grace** (2026-09-26, agent 1.85.2) — a Dell PXE-imaged with its CMOS clock at 2004 recorded its install (and its 30-day grace) in 2004; clockfix then set 2026 and WMI read `RemainingGracePeriod=0`, one reboot from the logon-blocking lockout. clockfix now asks `Win32_WindowsProductActivation` first and leaves the clock wrong, loudly, unless the move fits the grace; LICSTATUS reports the WMI verdict (`wpa`) and safe-reboot.py refuses on it (LICSTATUS on 1.85.2+, read-only `wmic … get` on older agents) | `native/test_clockguard.c` (TRUE-SOURCE `agent/shared/clockguard.h`), `python/test_clockfix_activation_and_audio.py`, `python/test_safe_reboot_activation.py` |
| **a sound card "installed - device working" with no wave device** (2026-09-26, agent 1.85.2) — the SoundMAX INF's `WDMAUDIO.Registration` RunOnce entries (streamci registrations for sysaudio/kmixer/wdmaud) were consumed without running, so XP's kernel audio stack never existed; the agent now re-applies the box's own wdmaudio.inf registration (registry only), runs just those entries, and reports the wave-device count after | `native/test_audiofix.c` (TRUE-SOURCE `agent/shared/audiofix.h`), `python/test_clockfix_activation_and_audio.py` |
