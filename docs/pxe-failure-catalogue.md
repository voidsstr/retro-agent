...
**Status 21 means the setup loader's NETWORK read failed.** This was verified
from the binary on 2026-09-25: `NetOpen` in setupldr returns 21 on any TFTP
failure, including file-not-found. A setupldr booted from a local disk that
cannot find the file returns 14. So this message always comes from a machine
that PXE-booted a setup loader and could not fetch `txtsetup.sif` over TFTP.
Two causes have been seen, and they need different fixes:

1. **A second PXE server answered the machine.** On 2026-09-25 a Dell
   Dimension 4600 hit this three times. Text mode finished every time and the
   box rebooted to boot its disk. This server correctly HELD it, and a stale
   copy of this server still scheduled on **whitebeast (`.249`)** answered
   instead. Its 302-byte `winnt.sif` pointed at the retired `XPSP3-PXE` tree,
   which does not exist. The tell: the message appears right after a `HOLD`
   line in our log, and **our log shows no TFTP request from that client**.
   Check it:
   `python3 scripts/pxe/pxe_rogue.py` (exit 1 names the other server). Since
   2026-09-25 `pxe_server.py` runs this probe at startup and every 10 minutes
   and logs a banner, and a Windows copy refuses to start next to another
   server. On whitebeast the rogue server is the `RetroPXE` scheduled task
   (runs as SYSTEM, needs an elevated shell to stop).
2. **The NAS was down** - see below.
...
### An injected storage driver claimed a controller in the wrong MODE

Found 2026-09-25 while chasing the Dell failure above. It was a real defect,
but it was **not** that failure's cause. `inject-massstorage.py` dropped the
`&CC_` class qualifier from every INF id. It wrote
`PCI\VEN_8086&DEV_24D1 = "iaStor2"` from `M021\dpsI2.inf`, whose model line
names the ICH5 SATA controller only as `...&DEV_24D1&CC_0106`, which is AHCI.
The bare `VEN&DEV` is one of every PCI device's compatible ids and outranks
retail's `PCI\CC_0101 = "pciide"`. So an IDE-mode ICH5 was bound to Intel's
RAID driver. **59 injected entries** had this shape: ICH6 `2651 -> iaStor3`,
nForce `nvgts`/`nvatabus4IN`, and Marvell `mv91xx`.

The generator now keeps the class qualifier (`parse_driver()`), and the live
...
also carries hand fixes and 14 force-loaded `[SCSI.Load]` drivers that a
regeneration from `.preinject` would drop. Test:
`tests/test_pxe_massstorage_class.py`.

**A text-mode reboot with no countdown is normal.** Unattended text mode with
`WaitForReboot=No` goes straight from `SpDone` to a reboot. A timeline of
"copy for 13-16 minutes, one `HOLD`" is a finished text mode, not a crash.
`SETUPREG.HIV` has no `CrashControl`, so a text-mode bugcheck would stay on
a blue screen.
