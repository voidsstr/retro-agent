...
*Rendered 2026-09-01 11:22 from records in `/mnt/retro-share/Utility/Retro Automation/fleet-inventory`.*
...
|----|----------|-------|----------|-------|-----|-----|-------------|----|
| 192.168.1.123 | NSC-B20C188E96D | current | 1 h ago | 1.79.4 | AMD Athlon(tm) 64 Processor 4000+, 2402 MHz | 2047 MB | ATI Radeon HD 3850 AGP (`1002:9515`), 512 MB, sm3.0 | Windows XP |
| 192.168.1.124 | NSC-9871C0E9964 | current | 26 min ago | 1.79.4 | AMD Athlon(tm) XP 1800+, 1503 MHz | 511 MB | NVIDIA GeForce2 GTS/GeForce2 Pro (Microsoft Corporation) (`10DE:0150`), 32 MB, tnl | Windows XP |
...
| 192.168.1.145 | DELL | current | 38 min ago | 1.79.4 | Intel(R) Core(TM) i5-2400 CPU @ 3.10GHz, 3092 MHz x4 | 3316 MB | NVIDIA GeForce 8400GS (`10DE:10C3`), 512 MB, sm3.0 | Windows XP |
...
| 192.168.1.240 | USER-41EA3B3330 | current | 40 min ago | 1.79.4 | AMD Athlon(tm) 64 Processor 3300+, 2403 MHz | 1022 MB | RADEON X800 Series (`1002:4A4B`), 256 MB, sm2.0 | Windows XP |
| 192.168.1.246 | ADMIN-PC | current | 42 min ago | 1.79.4 | Intel(R) Core(TM) i5-2400 CPU @ 3.10GHz, 3093 MHz x4 | 3317 MB | AMD Radeon HD 5450 (`1002:68F9`), 512 MB, sm3.0 | Windows 7 |

---

## 192.168.1.123 - NSC-B20C188E96D

**As this box reported itself at 2026-09-01 09:43** (1 h ago). Every value below is that snapshot; restart its agent to take a fresh one.
...
| Agent | 1.79.4 (the version that WROTE this record, at 2026-09-01 09:43) |
...
**As this box reported itself at 2026-09-01 10:56** (26 min ago). Every value below is that snapshot; restart its agent to take a fresh one.
...
| field | value |
|-------|-------|
| CPU | AMD Athlon(tm) XP 1800+, 1503 MHz |
| CPU id | family 6 model 8 stepping 1, vendor `AuthenticAMD` |
| Instruction set | fpu, mmx, cmov, 3dnow, sse |
| RAM | 511 MB |
| Display GPU | NVIDIA GeForce2 GTS/GeForce2 Pro (Microsoft Corporation) (`10DE:0150`), 32 MB, tnl |
| GPU driver | 5.6.7.3 (4-7-2004) |
| Display mode | 800x600 (registry) |
| OS | Windows XP Service Pack 3 (5.1.2600) |
| DirectX | 4.09.00.0904 |
| Disks | C: 4/6 GB free |
| Agent | 1.79.4 (the version that WROTE this record, at 2026-09-01 10:56) |
| Profile hash | `ad9dfa5da5fc78bd` |

**No 3dfx silicon.** `Enum\PCI` carries no `VEN_121A` key, and a physically fitted card enumerates there even with no driver bound - so this is the decisive read, not merely "undriven".

Network: 192.168.1.124 - MAC 00-D0-B7-40-96-A9

> **Clock skew:** this box's own clock reads 2003-04-02 11:47:31, 12316209 minutes behind the time the record landed here. Staleness above is judged by this host's clock, not the box's.

---

## 192.168.1.133 - P3-DUAL

**As this box reported itself at 2026-09-01 10:03** (1 h ago). Every value below is that snapshot; restart its agent to take a fresh one.
...
| Agent | 1.79.4 (the version that WROTE this record, at 2026-09-01 10:03) |
...
**As this box reported itself at 2026-09-01 10:04** (1 h ago). Every value below is that snapshot; restart its agent to take a fresh one.
...
| Agent | 1.79.4 (the version that WROTE this record, at 2026-09-01 10:04) |
...
**As this box reported itself at 2026-09-01 10:43** (38 min ago). Every value below is that snapshot; restart its agent to take a fresh one.
...
| Agent | 1.79.4 (the version that WROTE this record, at 2026-09-01 10:43) |
...
**As this box reported itself at 2026-09-01 10:41** (40 min ago). Every value below is that snapshot; restart its agent to take a fresh one.
...
| Agent | 1.79.4 (the version that WROTE this record, at 2026-09-01 10:41) |
...
> **Clock skew:** this box's own clock reads 2026-09-01 07:41:21, 180 minutes behind the time the record landed here. Staleness above is judged by this host's clock, not the box's.

---

## 192.168.1.246 - ADMIN-PC

**As this box reported itself at 2026-09-01 10:40** (42 min ago). Every value below is that snapshot; restart its agent to take a fresh one.
...
| Agent | 1.79.4 (the version that WROTE this record, at 2026-09-01 10:40) |
...
- `XPBUILD.json` - XPBUILD, 10.0.2.15, last seen 2026-09-01 06:56
